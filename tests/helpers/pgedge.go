...
// RunPgedgeHelmIntegrationTests runs the full pgedge-helm integration test suite
// against a cluster that already has CNPG and cert-manager installed.
// Each integration test manages its own helm install/uninstall cycle.
func RunPgedgeHelmIntegrationTests(t *testing.T, kubeconfigPath, kubeContext, pgedgeHelmPath, postgresImage, initSpockImage string, totalTimeout time.Duration) {
	t.Helper()

	t.Logf("Running pgedge-helm integration tests (context: %s, chart: %s)", kubeContext, pgedgeHelmPath)

	ensureGotestsum(t)

	// Per-test TIMEOUT: subtract a 10-minute buffer for go test process overhead.
	...
	cmd.Env = append(os.Environ(),
		...
		"NAMESPACE=default",
		fmt.Sprintf("CHART_REF=%s", pgedgeHelmPath),
	...
	if initSpockImage != "" {
		cmd.Env = append(cmd.Env, fmt.Sprintf("INIT_SPOCK_IMAGE=%s", initSpockImage))
	}
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
